...
module HandHolder() {
    difference() {
        color("orange") roundedCube([handholder_box.x, handholder_box.y, handholder_box.z + 1.5], 2, false, false);
        color("red") translate([(handholder_box.x - battery_dia) / 2, fatness + 1.5, fatness + .5]) {
            cube([battery_dia + inset, battery_length + inset, 100]);
        }
        translate([- .01, - .01, handholder_box.z]) cube([100, 100, 100]);
...
//translate([fatness + support_inset, 3, 19]) // for modelling
translate([0, 55, 0]) // for print
    difference() {
...
// cover
//translate([0, 0, height + inset / 2]) {// for modelling
translate([102, main_box.y, fatness]) rotate([180]) {// for print
    union() {
        height = 15;
        # cube([main_box.x, main_box.y, fatness]);
        translate([fatness + inset / 2, fatness + inset / 2, - height + .01]) difference() {
            f = 1;
            outter = [main_inner_box.x - inset, main_inner_box.y - inset, height];
            color("blue") cube(outter);
            color("green") translate([f, f, - .01]) cube([outter.x - 2 * f, outter.y - 2 * f, 100]);

            // board
            translate([outter.x - f - .01, 13.9, - .01]) cube([f + .02, 30, height - 5]);

            // disp. support - right
            translate([outter.x - f - .01, - .01, - .01]) cube([f + .02, 14, height - 3]);

            // front - left
            translate([- .01, - .01, - .01]) cube([50, f + .02, height - 4]);
            // front - middle
            translate([28.8, - .01, - .01]) cube([display_board_size.x + 1, f + .02, height - 2]);
            // front - right
            translate([67, - .01, - .01]) cube([100, f + .02, height - 4]);
            // front - right support
            translate([main_inner_box.x - 3, - .01, - .01]) cube([100, f + .02, height - 3]);

            // disp. support - left
            translate([- .01, - .01, - .01]) cube([f + .02, 14.5, height - 5.5]);
        }
...
// batt cover
//translate([(main_box.x - handholder_box.x) / 2, - handholder_box.y + 2.9, handholder_box.z + inset / 2]) {// for modelling
translate([102, main_box.y + 5, fatness]) rotate([180, 0, 90]) {// for print

    union() {
        height = 6.5;

        # color("blue") translate([.01, .01, - handholder_box.z + .9]) difference() {
            roundedCube([handholder_box.x - .01, handholder_box.y - .01, handholder_box.z + 1.5], 2, false, false);
            translate([- .01, - .01, - .01]) cube([100, 100, handholder_box.z - 1.15]);
            translate([- .01, 68.7, - .01]) cube([100, 100, 100]);
        }

        translate([1.7, 3, - height - .25]) difference() {
            f = 1;
            outter = [battery_dia, battery_length, height];
            color("violet") cube(outter);
            color("green") translate([f, f, - .01]) cube([outter.x - 2 * f, outter.y - 2 * f, 100]);
            color("green") translate([f, - f - .01, - .01]) cube([outter.x - 2 * f, 100, height - 1]);
        }
